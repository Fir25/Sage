...
const handleCloseannuller=()=>{
 setopenannuller(false)
  ...
  const { data: Conges = [],isloading:ll,error:ee } = useFetch(url + "TestConges/" + iduserinfo + "/" + iddep)
...
Conges.filter(x=>(x.validationrh==1 ) && (x.idrh2==iduserinfo)).map(c =>
...
export default CrudCongéoperation;