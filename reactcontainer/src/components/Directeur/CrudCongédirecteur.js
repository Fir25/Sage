...
function CrudCongédirecteur() {
  ...
  $(document).ready(function () {
    ...
    $('#congesTable thead tr:eq(1) th').each( function (i) {
    var title = $(this).text();
...
var table=  $('#congesTable').DataTable({
 ...
 const [selectedcong, setselected] = useState(null)
  ...
  const handleClickOpenrefus = (id, iduser, datedebut, datefin, validationn, validationrhh, chefid, rhid, emailemp, emailche, user_namef, last_namef,emailrhf,idrh2f,emailrh2f,validationrh2f,remarque1ref,remarque2ref) => {
    setopenrefus(true);
    setIdcongeRefus(id)
    setIdUserr(iduser)
    ...
    setRemarqueRh1ref(remarque1ref)
    seRemarquerh2f(remarque2ref)
  };
  ...
  const handleClickOpenAnnuller=(id, iduser, datedebut, datefin, validationn, validationrhh, chefid, rhid, emailemp, emailche, user_namef, last_namef,emailrhf,idrh2f,emailrh2f,validationrh2,remarque1ann,remarque2ann)=>{
    ...
    setRemarqueRH1Ann(remarque1ann)
    setRemarqueRH2Ann(remarque2ann)
  }
const handleCloseannuller=()=>{
 setopenannuller(false)
  ...
  const onClick = (id, iduserr, datedebut, datefin, validationn, validationrhh, idchef, idrh, emailemploye, emailchef, user_name, last_name,emailrh,idrh2,emailrh2,validationrh22) => {
    ...
    if (DRH == true ||   role=="admin") {//ken employé eli d5al RH (3ando role rh et appartient à département x ou rh de département x)
     

      if (validationrh22 == 0 || validationrh22 == "" ||  validationrh22 == null  ||  validationrh22 == undefined  ){
        sendMail(emaildrh,"IPS Time:  Avis Favorable pour la demande du congé "+user_name ,"Bonjour, La demande de congé du collaborateur "+user_name +"  du "+datedebut+" au "+datefin+"  est validée par avec un avis favorable !")
      ...
      {/**  else        if ( validationrhh == 0 || validationrhh == "" ||  validationrhh == null  ||  validationrhh == undefined) {
    ...
    else if ((idrh2==iduserinfo)) {//ken employé eli d5al RH (3ando role rh et appartient à département x ou rh de département x)
      //array1.find(element => element > 10);
   
      if (   validationrh22 == 0 || validationrh22 == "" ||  validationrh22 == null  ||  validationrh22 == undefined) {
        sendMail(emailemploye, "IPS Time:Avis Favorable pour la demande du congé", "Bonjour " + user_name +  ",  Votre manager 2 a validé le congé de votre demande du " + datedebut + " au " + datefin + " ")
        sendMail(emailchef,"IPS Time:  Avis Favorable pour la demande du congé  "+user_name ," Bonjour, La demande de congé du collaborateur "+user_name +"  du "+datedebut+" au "+datefin+"  est validé avec un avis favorable  !")  
        ...
        sendMail(emailrh,"IPS Time:  Avis Favorable  pour la demande du congé  "+user_name ," Bonjour, La demande de congé du collaborateur "+user_name +"  du "+datedebut+" au "+datefin+"  est validé avec un avis favorable   !")  
        sendMail(emaildrh,"IPS Time:  Avis Favorable pour la demande du congé  "+user_name ," Bonjour, La demande de congé du collaborateur "+user_name + "  du "+datedebut+" au "+datefin+"  est validé avec un avis favorable   !")  

        
        const validationrh2 = 2
        if (validationn == '') {
          const validation = 'en cours'
          return ValiderConge(id, iduserr, datedebut, datefin, validation, validationrhh,validationrh2)
  ...
  const onClickRefuser = (id, iduserr, datedebut, datefin, validationn, validationrhh, idchef, idrh, emailemploye, emailchef, user_name, last_name,emailrhf,idrh2f,emailrh2f,validationrh2f,remarque1ref,remarque2ref) => {
    ...
    if (DRH==true ||   role=="admin") {
      if ( validationrh2f == 0 || validationrh2f == "" ||  validationrh2f == null  ||  validationrh2f == undefined) {
        ...
        //  sendMail(emailchef,"IPS Time:  Avis défavorable par  DRH pour la demande du congé "+user_name + "   " + last_name ,"Bonjour, La demande de congé du collaborateur "+user_name + "   " + last_name+"  du "+datedebut+" au "+datefin+"  est refusé avec un avis défavorable par  DRH !")
         ...
         // sendMail(emailrhf,"IPS Time:  Avis défavorable par DRH pour la demande du congé  "+user_name + "   " + last_name ,"Bonjour, La demande du congé du collaborateur "+user_name + "   " + last_name+"  du "+datedebut+" au "+datefin+"  est refusé avec un avis défavorable par DRH !")
           ...
           if (validationn == '') {
             const validation = 'en cours'
             return RefuserConge(id, iduserr, datedebut, datefin, validation, validationrhh,validationrh2ff,remarque1ref,remarquerefuser)
      ...
      {/**    else       if (        validationrhh == 0 || validationrhh == "" ||  validationrhh == null  ||  validationrhh == undefined) {
       ...
       // sendMail(emailemploye, "IPS Time: Avis défavorable pour la demande du congé", "Bonjour  " + user_name + " " + last_name + ",  Votre RH a refusé le congé de votre demande du " + datedebut + " au " + datefin + ".  L'avis de chef est importante pour la confirmation définitive!")
        ...
        sendMail(emailrh2f,"IPS Time:  Avis défavorable  pour la demande du congé  "+user_name,"Bonjour, La demande du congé du collaborateur "+user_name +"  du "+datedebut+" au "+datefin+"  est refusé avec un avis défavorable  !")

        const validationrh = 3

        if (validationn == '') {
          const validation = 'en cours'
          return RefuserConge(id, iduserr, datedebut, datefin, validation, validationrh,validationrh2f,remarquerefuser,remarque2ref)
      ...
      if (validationrh2f == 0 || validationrh2f == "" ||  validationrh2f == null  ||  validationrh2f == undefined) {
        ...
        sendMail(emailchef,"IPS Time:   Avis défavorable  pour la demande du congé  "+user_name  ," Bonjour, La demande de congé du collaborateur "+user_name +"  du "+datedebut+" au "+datefin+"  est refusé avec un avis défavorable  ! <br/>  Remarque de la décision de refus  :"+remarquerefuser)
        sendMail(emailrh2f,"IPS Time:   Avis défavorable pour la demande du congé"+user_name  ," Bonjour, La demande de congé du collaborateur "+user_name +"  du "+datedebut+" au "+datefin+"  est refusé avec un avis défavorable ! <br/>   Remarque de la décision de refus  :"+remarquerefuser)
        sendMail(emailrhf,"IPS Time:   Avis défavorable  pour la demande du congé  "+user_name ," Bonjour, La demande de congé du collaborateur "+user_name + "  du "+datedebut+" au "+datefin+"  est refusé avec un avis défavorable  ! <br/>   Remarque de la décision de refus  :"+remarquerefuser)
     
        const validationrh2f =4

        if (validationn == '') {
          const validation = 'en cours'
          return RefuserConge(id, iduserr, datedebut, datefin, validation, validationrhh,validationrh2f,remarque1ref,remarquerefuser)
      ...
      alert('L employé de ce congé a un manager ')
    }

  };

  const OnClickAnnuller = (id, iduserr, datedebut, datefin, validationn, validationrhh, idchef, idrh, emailemploye, emailchef, user_name, last_name,emailrhf,idrh2f,emailrh2f,validationrh2anuuler,remarque1ann,remarque2ann) => {


    if (DRH==true ||   role=="admin") {
      if (     validationrh2anuuler == 0 || validationrh2anuuler == "" ||  validationrh2anuuler == null  ||  validationrh2anuuler == undefined || validationrh2anuuler==2) {
        ...
        // sendMail(emailchef,"IPS Time:  Avis défavorable par  rh pour la demande du congé "+user_name + "   " + last_name ,"Bonjour, La demande de congé du collaborateur "+user_name + "   " + last_name+"  du "+datedebut+" au "+datefin+"  est annulé avec un avis défavorable par  rh !")
         ...
         if (validationn == '') {
           const validation = 'en cours'
           return AnnullerConge(id, iduserr, datedebut, datefin, validation,validationrhh, validationrh2f,remarque1ann,remarque)
      ...
      {/**  else       if (   validationrhh == 0 || validationrhh == "" ||  validationrhh == null  ||  validationrhh == undefined) {
       sendMail(emailrh2f,"IPS Time:  Avis défavorable  pour la demande du congé "+user_name  ,"Bonjour, La demande de congé du collaborateur "+user_name +"  du "+datedebut+" au "+datefin+"  est annulé avec un avis défavorable  !")
       // sendMail(emailemploye, "IPS Time: Avis défavorable pour la demande du congé", "Bonjour  " + user_name + " " + last_name + ",  Votre RH a annulé le congé de votre demande du " + datedebut + " au " + datefin + ".  L'avis de chef est importante pour la confirmation définitive!")
       ...
       // sendMail(emailchef,"IPS Time:  Avis défavorable par  rh pour la demande du congé "+user_name + "   " + last_name ,"Bonjour, La demande de congé du collaborateur "+user_name + "   " + last_name+"  du "+datedebut+" au "+datefin+"  est annulé avec un avis défavorable par  rh !")
        ...
        if (validationn == '') {
          const validation = 'en cours'
          return AnnullerConge(id, iduserr, datedebut, datefin, validation,validationrh, validationrh2anuuler)
      ...
      if (   validationrh2anuuler == 0 || validationrh2anuuler == "" ||  validationrh2anuuler == null  ||  validationrh2anuuler == undefined || validationrh2anuuler==2) {
        sendMail(emailemploye, "IPS Time: Avis défavorable pour la demande du congé", "Bonjour  " + user_name  + ", Votre demande de congé de   " + datedebut + " au " + datefin + "a été annulé avec un avis défavorable !  <br/>   Remarque de la décision d'annulation :"+remarque)
        ...
        const validationrh2anuuler = 6

        if (validationn == '') {
          const validation = 'en cours'
          return AnnullerConge(id, iduserr, datedebut, datefin, validation, validationrhh,validationrh2anuuler,remarque1ann,remarque)
  ...
  const { data: Conges = [],isloading:ll,error:ee } = useFetch(url + "listvalidation/" + iduserinfo + "/motifConge" )
  ...
  // or response.text() or response.blob() depending on the response type
...
<td>{ c.validationrh==0 ? "en_attente" : c.validationrh==3  ?"1er refus" : c.validationrh==5? "1er annullation":c.validationrh==1? "1ére validation ":"" }</td>
 ...
 {c.validation !== "4" && c.validation !== "5" && c.validation !== "6" && (
          <a  className="btn-sm btn-success " onClick={() => {  handleClickOpenvalid() ; setselected(c.id) ; } }>
 ...
 >
  ...
  <DialogActions>
   ...
   <Button  onClick={() => { setopenvalid(false) ;  valider(selectedcong)  ; }}>
      oui
    </Button>
 ...
 <td>
 {c.validation !== "4" && c.validation !== "5" && c.validation !== "6" && (
              <a className="btn-sm btn-danger " onClick={() => {setselected(c.id) ; setopenrefus(true)}}>
 <ThumbDownIcon className={classes.icon}/>
    ...
    {c.validationrh2==2 && c.validationrh2 != 4?
...
{/**      <td>{c.nomchef}</td>
      ...
      <td>{c.datedebut}   {c.matindebut==true? "Matin": "Aprés-midi"}</td>
      <td>{c.datefin}   {c.matinfin==true? "Matin": "Aprés-midi"}</td>
      <td>{  c.validation==4   ?"validée" : c.validation==5 ? "annulé": c.validation==2? "validée par valideur 2 ":c.validation==1? "validée par valideur 1 ":c.validation==3? "validée par valideur 3 " : c.validation==0? "en_attente":"" }</td>
...
{c.validationrh2==2 && c.validationrh2 != 4?  <td>  <a className="btn-sm btn-warning" onClick={()=>handleClickOpenAnnuller(c.idconge, c.iduser, c.datedebut, c.datefin, c.validation, c.validationrh, c.chef_id, c.rh_id, c.emailemploye, c.email_chef, c.user_name, c.last_name,c.emailrh,c.idrh2,c.emailrh2,c.validationrh2,c.remarquerh1,c.remarquerh2)}><DoDisturbAltIcon className={classes.icon}  /></a>
...
>
 ...
 <DialogContent>
   <DialogContentText id="alert-dialog-description">
   Voulez-vous vraiment refuser ce congé ?
   </DialogContentText>
 ...
 <DialogActions>
   ...
   <Button onClick={() => { refus(selectedcong)}}>
...
<DialogActions>
 ...
 <Button onClick={() => { OnClickAnnuller(idcongeannuller, iduserrannuller, datedebutannuller, datefinannuller, validationnannuller, validationrhhannuller, idchefannuller, idrhannuller, emailemployeannuller, emailchefannuller, user_namefannuller, last_namefannuller,emailrhfannuller,idrh2fannuller,emailrh2fannuller,validationrh2annull,remarquerh1annul,remarquerh2annul) }}>  
...
export default CrudCongédirecteur;